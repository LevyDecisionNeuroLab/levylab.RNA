% LOGIT_CHOICE_PROB                Binary logit choice probability
% 
%     p = logit_choice_prob(vF,vA,pF,pA,AL,beta,model);
%
%     INPUTS
%     vF    - matrix of values of fixed option
%     vA    - matrix of values of ambiguous option
%     pF    - matrix of probabilities of fixed option
%     pA    - matrix of probabilities of ambiguous option
%     AL    - matrix of ambiguity levels
...
%     simon 2016/3/9 renamed from `choice_prob_ambigNrisk` & redocumented for clarity
%
% TODO: Rewrite the various models to use getSubjValue for their purposes

function p = logit_choice_prob(base,vF,vA,pF,pA,AL,beta,model);
...
%     p1 = 1 ./ (1 + exp(beta(1)*(uA-uF)));
    ...
    if (strcmp(model,'ambigNriskFixSlope'))
        uF = getSubjValue(vF,pF,zeros(size(vF)),beta(2),beta(1),model); %fixed non-ambiguous
        uA = getSubjValue(vA,pA,AL,beta(2),beta(1),model); % ambiguous
        slope = -1;
    elseif (strcmp(model,'ambigNrisk') || strcmp(model,'ambiguity') || strcmp(model,'ambigPower'))
        % this is the one we are currently using!
        uF = getSubjValue(base,vF,pF,zeros(size(vF)),beta(3),beta(2),model); %fixed non-ambiguous
        %uF = vF;
        uA = getSubjValue(base,vA,pA,AL,beta(3),beta(2),model); % ambiguous
        slope = beta(1);
