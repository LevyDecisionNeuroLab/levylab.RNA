...
#' @import tidyverse
#' @export
getModelFreeEstimates <- function(decision_data) {
    # Proportion of lottery choices per subject + per probability/value combination
    mf_risk_by.level.reward <- decision_data %>% filter(al == 0) %>% group_by(ID, p, val) %>% summarize(n = n(), proportion = mean(choice))
    head(mf_risk_by.level.reward)

    # Proportion of lottery choices per subject per probability level (without respect to value)
    mf_risk_by.level <- mf_risk_by.level.reward %>% summarize(prop_p = mean(proportion))
    head(mf_risk_by.level)

    # Proportion of risky choices per subject (without respect to probability level)
    mf_risk_all.levels <- mf_risk_by.level %>% summarize(risk_prop = mean(prop_p))
    mf_risk_all.levels

    # Proportion of lottery choices per subject + per ambiguity/value combination
    mf_ambig_by.level.reward <- decision_data %>% filter(al > 0) %>% group_by(ID, al, val) %>% summarize(n = n(), proportion = mean(choice))
    head(mf_ambig_by.level.reward)

    # Proportion of ambiguous choices per subject (without respect to value)
    mf_ambig_by.level <- mf_ambig_by.level.reward %>% summarize(proportion = mean(proportion))
    head(mf_ambig_by.level)

    # What proportion of all ambiguous choices did each subject select?
    mf_ambig_all.levels <-  mf_ambig_by.level %>% summarize(ambig_prop = mean(proportion))
    mf_ambig_all.levels
    ...
    mf_relative.ambig_all.levels <- group_by(mf_relative.ambig_by.level, ID) %>%
        summarize(ambig_prop_relative = mean(diff))
